...
int main(int argc, char *argv[])
{
    ...
    char *hexSeed = hexlify(seed,32);

	printf("Shuffling seed (in hex): %.64s\n\n", hexSeed);
	...
	counter = 0;
	printf("Scrambled Sboxes: [ \n");
	for (int i = 0; i < 16 ; i ++){
		printf("Sbox %d: [ ",i+1);

		for (int j = 0; j < 256; j++){
            shuffledSboxes[i][j] = sboxes[counter];
			printf("%x, ",sboxes[counter]);
			counter++;
		}
		printf("]\n");
	}
    ...
    uint8_t data[8] = {0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01};
    ...
    char *hexData = hexlify(data,8);
    printf("Encrypted data in hex format:  ");
    ...
    uint8_t* encryptedData = unhexlify(hexData,16);

    feistelRounds(encryptedData,false,shuffledSboxes);
